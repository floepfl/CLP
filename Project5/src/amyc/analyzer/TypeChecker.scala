    ...
    def genConstraints(e: Expr, expected: Type)(implicit env: Map[Identifier, Type]): List[Constraint] = {
      ...
      e match {
        ...
        case Equals(lhs, rhs) =>
          ...
          val lhsConstr = genConstraints(lhs, TypeVariable.fresh())
        ...
        case Times(lhs, rhs) =>
          genConstraints(lhs, IntType) ::: (Constraint(IntType, IntType, e.position) :: genConstraints(rhs, IntType))
        case And(lhs, rhs) =>
          genConstraints(lhs, BooleanType) ::: (Constraint(BooleanType, BooleanType, e.position) :: genConstraints(rhs, BooleanType))
        case Mod(lhs, rhs) =>
          genConstraints(lhs, IntType) ::: (Constraint(IntType, IntType, e.position) :: genConstraints(rhs, IntType))
        case Div(lhs, rhs) =>
          genConstraints(lhs, IntType) ::: (Constraint(IntType, IntType, e.position) :: genConstraints(rhs, IntType))
        ...
        case Neg(e) => genConstraints(e, IntType)
        case Not(e) => genConstraints(e, BooleanType)
        case Call(qname, args) =>
          ...
          if(funSig.isDefined) {
            ...
            val argsConstraint = argsAndTypes.flatMap{case(expr, t) => genConstraints(e, t)}
            Constraint(tpe, expected, e.position) :: argsConstraint
          } else {
            val argsAndConstr = args.zip(constrSig.get.argTypes)
            val argsConstraint = argsAndConstr.flatMap{case(expr, t) => genConstraints(e, t)}
            Constraint(tpe, expected, e.position) :: argsConstraint
          }
        case Sequence(s1, s2) =>
